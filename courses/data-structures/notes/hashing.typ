...
#theorem("Expected Collisions (Chain Length)")[
  Let $H$ be a $c$-universal family of functions from $U$ to $[m]$, $X = {x_1, ..., x_n} subset U$ a set of items stored in the data structure, and $y in U without X$ another item not stored in the data structure. Then we have
...
#proof[
  We use Lemma M.
  1. *Analysis of Linear Functions in $ZZ_p$*: Consider the family $L' = {h_(a,b)(x) = (a x + b) mod p}$ mapping $ZZ_p$ to $ZZ_p$.
  ...
  3. *(2, 1)-Independence*: Since there is exactly 1 solution out of $p^2$ total pairs $(a, b)$, the probability $P[h(x)=r and h(y)=s] = 1/p^2$. This means $L'$ is $(2, 1)$-independent (where $c=1$).
  4. *Conclusion*: The family $L$ is obtained by taking $L'$ modulo $m$. By Lemma M with $c=1$, $L$ is $2(1) = 2$-universal and $(2, 4(1)) = (2, 4)$-independent.
]
...
#proof[
  ...
  Since there are $p^k$ possible polynomials (choices of coefficients $bold(t)$) and exactly one satisfies the condition, the probability that a random polynomial from $P_k$ has this property is $1/p^k$.

  The family $P_k$ maps $ZZ_p$ to $ZZ_p$, so the target range size is $r = p$. It is $(k, 1)$-independent, so $c=1$. Lemma K states that if the range size $r$ (here $p$) is at least $2k m$, then taking the result modulo $m$ yields a family that is $(k, 2c)$-independent. Substituting $c=1$, we get $(k, 2)$-independence.
]
