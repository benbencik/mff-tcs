...
#table(
  ...
  [Amortized Efficiency: $O(log n)$ for all operations.],
  [Worst Case: $O(n)$ for a single operation (linear height possible).],

  [Locality of Reference: Frequently accessed nodes stay near the root.],
  [No benefit on randomized data],

  [Simplicity: No extra storage required for balance information.],
  [Modifies on Read: `Find` changes the tree structure (concurrency issues).],
)
